...
AS
    ...
    IF @current_version < @to_version
        BEGIN
            ...
            DECLARE cursor_migrate CURSOR FOR SELECT
                                                  upgrade_proc,
                                                  [version]
            ...
            FETCH NEXT FROM cursor_migrate
            ...
            WHILE @@FETCH_STATUS = 0
                BEGIN
                    ...
                    IF @version > @current_version
                        BEGIN
                            PRINT 'Upgrading from version ' + CONVERT(CHAR(2), @current_version) + ' to version ' +
                                  CONVERT(CHAR(2), @version);
                            -- Update version
            ...
            WHILE @@FETCH_STATUS = 0
                BEGIN
                    ...
                    IF @version - 1 < @current_version
                        BEGIN
                            PRINT 'Downgrading from version ' + CONVERT(CHAR(2), @current_version) + ' to version ' +
                                  CONVERT(CHAR(2), @version - 1)
                            -- Update version
                            SET @current_version = @current_version - 1;
                            PRINT 'Database version is now ' + CONVERT(CHAR(2), @current_version)
                        END
