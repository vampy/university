...
public:
     ...
     * @return bool
     */
    bool hasNext() { return nrEntries < map->nrEntries; }
...
public:
     ...
     * The map constructor
     */
    Map() { this->initBuckets(); }

    ~Map() { this->deleteBuckets(); }
     ...
     * @return bool
     */
    bool containsKey(string key) { return this->findInBucket(buckets->at(this->hashIndex(key)), key) != -1; }

    /**
     * Get the map itertor. You must eliberate the iterator yourself from memory with delete.
     * @return MapHash::Iterator<TValue>*
     */
    MapHash::Iterator<TValue>* getIterator() { return new MapHash::Iterator<TValue>(this); }

    /**
     * Checks if the map is empty
     * @return bool
     */
    bool isEmpty() const { return nrEntries == 0; }

    /**
     * Get the number of entries in the map
     * @return unsigned int
     */
    unsigned int getLength() const { return nrEntries; }
...
protected:
     ...
     * @return unsigned int
     */
    unsigned int hashIndex(string key) { return this->hash(key) % buckets->size(); }
    ...
    void rehash()
    {
        ...
        if (loadFactor > 0.8) // increase buckets
        {
            ...
            printDebug(
                "HashMap: rehash UP -> old_size = " + to_string(old_size) + ", new_size = " + to_string(new_size));
