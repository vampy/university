...
Ingredient* MemRepository::getById(const unsigned int id) const { return array->at(this->getIndexByID(id)); }

Ingredient* MemRepository::getByIndex(const unsigned int index) const { return array->at(index); }
...
unsigned int MemRepository::getLength() const { return array->size(); }
...
void MemRepository::toString() const
{
    ...
    for (size_t i = 0; i < array->size(); i++)
    {
        ...
        cout << setw(4) << it->getId() << setw(15) << it->getQuantity() << setw(30) << it->getName() << setw(30)
             << it->getProducer() << endl
             << endl;
    }
}
