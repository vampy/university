...
public:
    ...
    bool isValid(const Ingredient *ingredient) const
    {
        return ingredient->getName() == this->name;
    }
};

class FilterProducerEqual: public RepositoryFilter
{
protected:
    string producer;
public:
    FilterProducerEqual(string p) : producer(p) {}
    bool isValid(const Ingredient *ingredient) const
    {
        return ingredient->getProducer() == this->producer;
    }
};
...
public:
    ...
    bool isValid(const Ingredient *ingredient) const
    {
        return ingredient->getQuantity() > this->quantity;
    }
};
