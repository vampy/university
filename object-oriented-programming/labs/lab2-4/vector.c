...
void vector_init(Vector *vector, size_t element_size)
{
    ...
    // allocate memory
    vector->_data = calloc(VECTOR_INITIAL_CAPACITY, sizeof(void*));
}
...
{
    assert(index >= 0); assert(index < vector->_length);
...
void vector_double_capacity_if_full(Vector *vector)
{
    ...
    if(vector->_length >= vector->_capacity)
    {
        ...
        vector->_capacity *= 2;
        printf(", After=%d \n", vector->_capacity);
        vector->_data = realloc(vector->_data, sizeof(void*) * vector->_capacity);
    }
}

void *vector_new_element(Vector *vector)
{
    return calloc(1, vector->_element_size);
}
...
{
    vector_remove(vector, vector->_length - 1);
}
