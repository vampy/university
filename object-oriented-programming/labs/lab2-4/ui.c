...
#define VALIDATE_HOLIDAY(v_temp_holiday, v_operation)                                           \
    if (validate_type(v_temp_holiday.type) && validate_destination(v_temp_holiday.destination)) \
    {                                                                                           \
        v_operation;                                                                            \
    }                                                                                           \
    else                                                                                        \
...
int ui_start()
{
    ...
    while (1)
    {
        ...
        else if (command == 2)
        {
            ...
            puts("Update holiday");
            printf("Id: ");
            SAFE_SCANF(scanf("%d", &(temp_holiday.id)))
        ...
        else if (command == 6)
        {
            ...
            printf("Greater than price(1) or less than price(-1): -1 or 1: ");
            SAFE_SCANF(scanf("%d", &choice));
            if (choice == -1 || choice == 1 || choice == 0)
            {
                printf("Price: ");
                SAFE_SCANF(scanf("%f", &temp_price))
        ...
        else if (command == 7)
        {
            ...
            {
                printf("The undo history is EMPTY");
                print_eol();
            }
        }
